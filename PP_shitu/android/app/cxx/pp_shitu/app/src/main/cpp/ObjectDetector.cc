...
void ObjectDetector::LoadModel(const std::string& model_file, int num_theads, std::string cpu_power)
{
    MobileConfig config;
...
void ObjectDetector::Postprocess(const std::vector <cv::Mat> mats, std::vector <ObjectResult> *result, std::vector<int> bbox_num, bool is_rbox = false)
{
    result->clear();
    int start_idx = 0;
                             ...
                             std::vector<double> *times)
{
    ...
    {
        PicoDetPostProcess(
                result, output_data_list_, fpn_stride_, inputs_.im_shape_,
                inputs_.scale_factor_, score_threshold_,
                nms_threshold_, num_class, reg_max);
